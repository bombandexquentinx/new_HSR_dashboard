...
import AddonsModel from "../components/listing/AddonsModel";
...
const AddOns = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await listingService.getAddonsListing();
  ...
  const getEditData = async (id) => {
    try {
      const response = await axios.get(
        `https://homestyleserver.xcelsz.com/api/listings/getById/${id}/addons`,
  ...
  const columns = [
    ...
    { 
      ...
      headerName: "Whats Included", 
      flex: 1,
      minWidth: 100 ,
      renderCell: (params) => (
        <span className="capitalize">{params.value?.join(', ') || ''}</span>
      )
    },
    { 
      field: "overview", 
          ...
          <div className="sm:ml-64 p-4">
            <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h4 className="text-xl font-semibold text-gray-800">Addons Listings</h4>
                <Button
                  variant="dark"
