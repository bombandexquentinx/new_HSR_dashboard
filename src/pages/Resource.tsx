...
import ResourceModel from "../components/listing/ResourceModel";
...
const Resource = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await listingService.getResourceListing();
  ...
  const getEditData = async (id) => {
    try {
      const response = await axios.get(
        `https://homestyleserver.xcelsz.com/api/listings/getById/${id}/resource`,
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <img
          src={`https://homestyleserver.xcelsz.com${params.row.images[0]}`}
          alt="Display"
    ...
    { 
      field: "total_minutes_read", 
      headerName: "Total minutes Read", 
      width: 120,
      renderCell: (params) => `${params.value?.toLocaleString()} mins`
    },
    { 
      field: "category", 
      headerName: "Category", 
          ...
          <div className="sm:ml-64 p-4">
            <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h4 className="text-xl font-semibold text-gray-800">Resource Listings</h4>
                <Button
                  variant="dark"
