...
interface AmenitiesProps {
    ...
    setFormData: React.Dispatch<React.SetStateAction<Partial<PropertyListing> & {
        propertyAmenities: string;
        paymentOptions: string;
        localAmenities: string;
    }>>;
...
const Amenities: React.FC<AmenitiesProps> = ({ formData, setFormData, listingCate, notify, amenities, setAmenities }) => {
    ...
    const handleAddAmenity = useCallback(() => {
        if (amenityInput && distanceInput && !isNaN(Number(distanceInput)) && localAmenityCategory) {
            ...
            const currentAmenities = typeof formData.localAmenities === 'string'
                ? JSON.parse(formData.localAmenities || '{}')
    ...
    const handleAddCustomAmenities = useCallback(() => {
        console.log(JSON.parse(formData.propertyAmenities))
        if (amenityInput && !JSON.parse(formData.propertyAmenities)[amenityInput]) {
                ...
                <CardContent className="p-4 lg:p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2" role="list" aria-label="Local amenities list">
                            {Object.entries(JSON.parse(formData.localAmenities || '{}') as Record<string, number>).map(([name, distance]) => (
                                <div
                                    key={name}
                                    role="listitem"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-48 overflow-y-auto">
                        {(listingCate ? getPaymentoption(listingCate as ListingCategory) : []).map((option) => (
                            <label key={option} className="flex items-center border rounded-md p-2">
                                <Checkbox
                                    checked={Boolean((JSON.parse(formData.paymentOptions || '[]') as string[]).includes(option))}
